...
app.post('/cars', authMiddleware, ownerMiddleware, async (c) => {
...
app.patch('/cars/:id/status', authMiddleware, ownerMiddleware, async (c) => {
...
app.patch('/bookings/:id', authMiddleware, ownerMiddleware, async (c) => {
  const id = c.req.param('id');
  const { status, isApproved } = await c.req.json();
  if (status) await c.env.DB.prepare('UPDATE bookings SET status = ? WHERE id = ?').bind(status, id, id).run();
